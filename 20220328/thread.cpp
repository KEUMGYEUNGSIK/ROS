...
/*
1. 아규먼트인 thread 는 쓰레드가 성공적으로 생성되었을때 생성된 쓰레드를 식별하기 위해서 사용되는 쓰레드 식별자이다.
2. 아규먼트인 attr 은 쓰레드 특성을 지정하기 위해서 사용하며, 기본 쓰레드 특성을 이용하고자 할경우에 NULL 을 사용한다.
3. 아규먼트인 start_routine는 분기시켜서 실행할 쓰레드 함수이며
4. 아규먼는인 arg는 쓰레드 함수의 인자이다.
*/

int pthread_join(pthread_t th, void **thread_return);

/*
1. 아규먼트 th는 기다릴(join)할 쓰레드 식별자이다.
2. 아규먼트 thread_return은 쓰레드의 리턴(return) 값이다.
*/
...
여기에서는 pthread_create 호출후 detach 하는 방법을 설명하고 있는데, pthread_create 호출시에 쓰레드가 detach 되도록 할수도 있다. 이에 대한 내용은 pthread_attr_setdetachstate를 다루면서 설명하도록 하겠다.
*/
